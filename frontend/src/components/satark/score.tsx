...
export function ScoreMeter({ score, band, threshold = 80, className }: { score: number; band: Band; threshold?: number; className?: string }) {
  return (
    <div className={cn("flex items-center gap-3", className)}>
      ...
      <div
        className="relative h-1.5 min-w-20 flex-1 rounded-sm bg-sunken"
        role="meter"
        aria-valuemin={0}
        aria-valuemax={100}
        aria-valuenow={score}
      ...
      >
        <div className={cn("absolute inset-y-0 left-0 rounded-sm", FILL[band])} style={{ width: `${Math.min(score, 100)}%` }} />
