        ...
        <ol className="relative space-y-4 border-l border-rule pl-4">
          {entries.map((entry) => (
            <li key={entry.id} className="relative">
              <span className="absolute top-1.5 -left-[21px] size-2 rounded-sm bg-ink" aria-hidden />
              ...
              <p className="mt-1 flex flex-wrap items-center gap-2 text-[12px] text-ink-2">
                ...
                <code className="rounded-sm border border-rule px-2 py-0.5 font-mono text-[11px] text-ink" title={entry.hash}>{shortHash(entry.hash)}</code>
              </p>
            </li>
          ))}
        </ol>
